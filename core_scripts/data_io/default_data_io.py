...
class NIIDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if global_arg is not None:
            ...
            # print('m_opt_wav_handler:', self.m_opt_wav_handler)
        else:
        ...
        # self.filenames = []
        # self.data_lens = []
        # for i in range(len(self.m_seq_info)):
        #     data, filename, data_len = self.getdata(i)
        #     self.datas.append(data)
        #     self.filenames.append(filename)
        #     self.data_lens.append(data_len)
        # done
        return
    ...
    def getdata(self, idx):
        ...
        return in_data, tmp_seq_info.seq_name, tmp_seq_info.length
    ...
    def f_check_file_list(self):
        ...
        for tmp_d, tmp_e in zip(self.m_input_dirs, self.m_input_exts):
            ...
            # tmp_new_list.extend(nii_list_tools.common_members(tmp_list,
            #                                              self.m_file_list))
            for tmp in nii_list_tools.common_members(tmp_list, self.m_file_list):
                tmp_new_list.append(os.path.join(tmp_d, tmp)+tmp_e)
    ...
    def f_calculate_stats(self, flag_cal_data_len, flag_cal_mean_std):
        ...
        # print information
        load_cnt = 0
        total_cnt = len(self.m_file_list)
        ...
        for t_dir, t_ext, t_dim, t_reso, t_norm in \
                zip(tmp_dirs, tmp_exts, [tmp_dims[0]], tmp_reso, tmp_norm):
            ...
            for file_name in self.m_file_list:
                ...
                # get file path
                file_path = file_name
                    # nii_str_tk.f_realpath(t_dir, file_name, t_ext)
                if not nii_io_tk.file_exist(file_path):
                    nii_warn.f_die("%s not found" % (file_path))
...
class NIIDataSetLoader:
    ...
    def __init__(self, setname,
                 ...
                 input_dirs,
                 proto_dir,
                 input_exts, input_dims, input_reso, \
                 input_norm, \
                 output_dirs, output_exts, output_dims, output_reso, \
                 output_norm, \
                 stats_path, \
        ...
        # self.save_file = 'cache_{}.npy'.format(setname)
        # if os.path.exists(self.save_file):
        #     self.datas, self.labels, self.data_length = torch.load(self.save_file)
        #     print('data have lodaed')
        #     return
        ...
        # 加载数据并排序
        seq_info_buff = self.m_dataset.m_seq_info
        self.transform = transforms
        f_load_data = lambda x, y: _data_reader(x, y, 'EN')
        self.input_dirs = input_dirs
        self.input_exts = input_exts
        self.input_dims = input_dims
        self.input_reso = input_reso
        self.load_data = f_load_data
        self.input_all_dim = sum(input_dims)
        self.output_dirs = output_dirs
        self.output_exts = output_exts
        self.output_dims = sum(output_dims)
        filename_lable = self.protocol_parse(proto_dir)
        self.labels = []
        labels_buff = []
        self.seq_info = []
        for info in seq_info_buff:
            file_name = os.path.basename(info.seq_name)
            labels_buff.append(filename_lable[file_name])
        sort_lable_indix = np.argsort(labels_buff)
        for i in sort_lable_indix:
            self.labels.append(labels_buff[i])
            self.seq_info.append(seq_info_buff[i])



        # self.datas = self.m_dataset.datas
        # # self.datas = Parallel(n_jobs=4, prefer='threads')(delayed(self.transform)(x) for x in
        # #                                                   self.datas)
        # self.filenames = self.m_dataset.filenames
        # self.data_length = self.m_dataset.data_lens
        # self.labels = []
        # for filename in self.filenames:
        #     label = filename_lable[filename]
        #     self.labels.append(label)
        #
        # # torch.save((self.datas, self.labels, self.data_length), self.save_file)
        # sort_lable_indix = np.argsort(self.labels)
        # self.data_buff = []
        # self.data_length_buff = []
        # self.labels_buff = []
        # for i in sort_lable_indix:
        #     self.data_buff.append(self.datas[i])
        #     self.data_length_buff.append(self.data_length[i])
        #     self.labels_buff.append(self.labels[i])
        #
        # self.datas = []
        # self.labels = []
        # self.data_length = []
        ...
        #
        #     start_idx = self.labels_buff.index(i)
        #     data_x = Parallel(n_jobs=4, prefer='threads')(delayed(self.transform)(x) for x in
        #                                                   self.data_buff[start_idx:start_idx+args.labels_per_class_train])
        #     self.datas.extend(data_x)
        #     self.labels.extend(self.labels_buff[start_idx:start_idx + args.labels_per_class_train])
        #     # self.filenames_per.append(self.filenames[start_idx[0]:start_idx[0] + args.labels_per_class_train])
        #     self.data_length.extend(self.data_length_buff[start_idx:start_idx + args.labels_per_class_train])
        # torch.save((self.datas, self.labels, self.data_length), self.save_file)

        return

    def protocol_parse(self, protocol_filepath):
        ...
        for row in temp_buffer:
            if 'N01' in row:
                data_buffer[row.split(',')[0]] = 0
            elif 'A01' in row:
                data_buffer[row.split(',')[0]] = 18
            elif 'A02' in row:
                data_buffer[row.split(',')[0]] = 11
            elif 'A03' in row:
                data_buffer[row.split(',')[0]] = 12
            elif 'A04' in row:
                data_buffer[row.split(',')[0]] = 13
            elif 'A05' in row:
                data_buffer[row.split(',')[0]] = 14
            elif 'A06' in row:
                data_buffer[row.split(',')[0]] = 15
            elif 'A07' in row:
                data_buffer[row.split(',')[0]] = 16
            elif 'A08' in row:
                data_buffer[row.split(',')[0]] = 17
            elif 'A09' in row:
                data_buffer[row.split(',')[0]] = 10
            elif 'A10' in row:
                data_buffer[row.split(',')[0]] = 19
            elif 'A11' in row:
                data_buffer[row.split(',')[0]] = 20
            elif 'A12' in row:
                data_buffer[row.split(',')[0]] = 21
            elif 'A13' in row:
                data_buffer[row.split(',')[0]] = 22
        return data_buffer
