...
def traindummy(epoch, net, dce):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_iterator):
        ...
        dummylogit, _ = dummypredict(net, dce, laterhalfinputs)  # 输出batch_size,1的向量
        ...
        l2b1, _ = latter2blockclf1(net, dce, mixed_embeddings)
        l2b2, _ = latter2blockclf2(net, dce, mixed_embeddings)
...
def valdummy(epoch, net, dce, mainepoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_iterator):
            ...
            dummylogit, _ = dummypredict(net, dce, inputs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--novel', nargs='+', type=int, default=[15])
    ...
    model_dir = 'proto_l1_{}_l2_{}_a_{}_d_{}_s_{}_de_{}'.format \
