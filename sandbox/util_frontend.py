...
class DceLoss(torch_nn.Module):
    def __init__(self, n_classes, feat_dim, init_weight=True):
        ...
        self.centers = torch_nn.Parameter(torch.randn(self.feat_dim, self.n_classes), requires_grad=True)
        self.radius = torch.nn.Parameter(torch.zeros(self.n_classes), requires_grad=True)
        if init_weight:
            self.__init_weight()
    ...
    def modefied_centers(self, nb_add=1):
        # self.centers_1 = copy.deepcopy(self.centers)
        # self.centers_1.requires_grad = False
        ...
        # self.add_center = torch_nn.Parameter(add_center, requires_grad=True)
        add_center = torch.randn(self.feat_dim, nb_add).cuda()
        add_center = torch_nn.init.kaiming_normal_(add_center)
        centers = torch.cat((self.centers, add_center), dim=1)
    ...
    def forward(self, x):
        ...
        return self.centers, -dist, self.radius
